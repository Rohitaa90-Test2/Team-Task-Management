...
export const ProjectDetail = () => {
  ...
  const [taskForm, setTaskForm] = useState({
    title: '', description: '', status: 'TODO', dueDate: '', assignedToId: null
  });
  ...
  const handleUpdateMemberRole = async (memberId, newRole) => {
    try {
      await projectAPI.updateMemberRole(id, memberId, newRole);
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-100 py-8">
        <div className="max-w-7xl mx-auto px-4">
          <button onClick={() => navigate('/projects')} className="text-blue-600 hover:text-blue-700 mb-6 block">
            ← Back to Projects
          </button>

          {/* Project Header */}
          <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
            <h1 className="text-2xl md:text-4xl font-bold text-gray-800 mb-2">{project?.name}</h1>
            <p className="text-gray-600 mb-6">{project?.description}</p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div><p className="text-gray-600 text-sm">Members</p><p className="text-2xl font-bold text-gray-800">{project?.members?.length || 0}</p></div>
              <div><p className="text-gray-600 text-sm">Total Tasks</p><p className="text-2xl font-bold text-gray-800">{tasks.length}</p></div>
              <div><p className="text-gray-600 text-sm">In Progress</p><p className="text-2xl font-bold text-yellow-600">{tasks.filter(t => t.status === 'IN_PROGRESS').length}</p></div>
              <div><p className="text-gray-600 text-sm">Completed</p><p className="text-2xl font-bold text-green-600">{tasks.filter(t => t.status === 'DONE').length}</p></div>
            </div>
          </div>
          ...
          <div className="flex gap-4 mb-8 border-b border-gray-300">
            <button
              ...
              className={`px-4 md:px-6 py-3 font-semibold transition ${activeTab === 'tasks' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600 hover:text-gray-800'}`}
            >Tasks</button>
            <button
              onClick={() => setActiveTab('members')}
              className={`px-4 md:px-6 py-3 font-semibold transition flex items-center gap-2 ${activeTab === 'members' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600 hover:text-gray-800'}`}
            ><Users size={18} /> Members</button>
          </div>
          ...
          {activeTab === 'tasks' && (
            <div className="bg-white rounded-lg shadow-lg p-4 md:p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl md:text-2xl font-bold text-gray-800">Tasks</h2>
                {isAdmin && (
                  <button
                  ...
                  ><Plus size={18} /> New Task</button>
                )}
              </div>

              {showTaskForm && isAdmin && (
                <form onSubmit={handleTaskSubmit} className="bg-gray-50 p-4 md:p-6 rounded-lg mb-6 space-y-4">
                  <div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <button type="button" onClick={() => setShowTaskForm(false)} className="bg-gray-400 hover:bg-gray-500 text-white px-6 py-2 rounded-lg transition">
                      Cancel
                    </button>
                  </div>
                </form>
          ...
          {activeTab === 'members' && (
            <div className="bg-white rounded-lg shadow-lg p-4 md:p-6">
              ...
              {showAddMemberForm && isAdmin && (
                <form onSubmit={handleAddMember} className="bg-gray-50 p-4 md:p-6 rounded-lg mb-6 space-y-4">
                  <div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <button type="button" onClick={() => setShowAddMemberForm(false)} className="bg-gray-400 hover:bg-gray-500 text-white px-6 py-2 rounded-lg transition">
                      Cancel
                    </button>
                  </div>
                </form>
...
const MemberCard = ({ member, isAdmin, currentUserId, onRemove, onUpdateRole }) => (
  <div className="border border-gray-300 p-4 rounded-lg flex flex-wrap justify-between items-center gap-3">
    <div>
      <h3 className="text-base font-semibold text-gray-800">{member.user.name}</h3>
    ...
    <div className="flex gap-2 items-center">
      {isAdmin && member.userId !== currentUserId ? (
        <>
          ...
          <button onClick={onRemove} className="p-2 bg-red-100 text-red-600 rounded hover:bg-red-200 transition">
            <Trash2 size={16} />
          </button>
        </>
      ) : (
        <span className="px-3 py-1 bg-gray-200 text-gray-700 rounded text-sm font-medium">{member.role}</span>
      )}
    </div>
  </div>
);
...
const TaskCard = ({ task, isAdmin, onStatusChange, onDelete }) => {
  const statusIcons = {
    TODO: <AlertCircle className="text-red-600 shrink-0" size={20} />,
    IN_PROGRESS: <Clock className="text-yellow-600 shrink-0" size={20} />,
    DONE: <CheckCircle2 className="text-green-600 shrink-0" size={20} />
  };
  const statusColors = {
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
        ...
        {isAdmin && (
          <div className="flex gap-2 shrink-0">
            ...
            <button onClick={onDelete} className="p-2 bg-red-100 text-red-600 rounded hover:bg-red-200 transition">
              <Trash2 size={16} />
            </button>
          </div>
        )}
